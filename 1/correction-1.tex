...
\newcommand{\HQ}{\obullet{\underline{\texttt{b}}}}
\newcommand{\s}{\texttt{a}}
\newcommand{\X}{\texttt{x}}
\newcommand{\e}{\texttt{\#}}
\newcommand{\st}{\colon\,}
...
$(\texttt{a},\e)$ in the previous explanation by $(q_i,\texttt{a},\e)$).
...
Let $S=(Q',\Sigma',\Gamma',\delta',q'_0,q'_{\text{accept}},q'_{\text{reject}})$.
$S$ must start with the same input as $M$ so $\Sigma'=\Sigma$. Let
$$\Gamma' = \Sigma \cup \{\,\obullet{\gamma} \st \s \in \Gamma\,\} \cup
\{\,\underline{\obullet{\gamma}} \st \s \in \Gamma\,\} \cup \{\,\0{}, \X, \e\,\}.$$
Start with $Q' = \{\,q'_0\,\}$. Below we define $\delta'$. Add each state used
in this definition to $Q'$.

\paragraph{Crossing of first symbol}
For all $\texttt{a} \in \Sigma$,
let $$\delta'(q'_0,\texttt{a})
= ((\text{init},\text{bound then copy}, \texttt{a}), \X, R).$$
Let $$\delta'(q'_0,\0{})
= ((\text{init},\text{bound then copy}, \0{}), \X, R).$$

\paragraph{Going to the first blank symbol}
For all $\texttt{a} \in \Sigma$,
for all $\texttt{b} \in \Sigma$,
let $$\delta'((\text{init},\text{bound then copy}, \texttt{a}), \texttt{b})
= ((\text{init},\text{bound then copy}, \texttt{a}), \texttt{b}, R).$$
For all $\texttt{b} \in \Sigma$,
let $$\delta'((\text{init},\text{bound then copy}, \0{}), \texttt{b})
= ((\text{init},\text{bound then copy}, \0{}), \texttt{b}, R).$$

\paragraph{Bound the input}
For all $\texttt{a} \in \Sigma$,
let $$\delta'((\text{init},\text{bound then copy}, \texttt{a}), \0{})
= ((\text{init},\text{copy}, \texttt{a}), \e, R).$$
Let $$\delta'((\text{init},\text{bound then copy}, \0{}), \0{})
= ((\text{init},\text{copy}, \0{}), \e, R).$$

\paragraph{Copy first symbol}
For all $\texttt{a} \in \Sigma$,
let $$\delta'((\text{init},\text{copy}, \texttt{a}), \0{})
= ((\text{init},\text{back}), \HA, L).$$
Let $$\delta'((\text{init},\text{copy}, \0{}), \0{})
= ((\text{init},\text{back}), \BB, L).$$

\paragraph{Going back to the first uncopied symbol of the tape}
For all $\gamma \in \Gamma' \setminus\{\,\X\,\}$,
let $$\delta'((\text{init},\text{back}), \gamma)
= ((\text{init},\text{back}), \gamma, L).$$
Let $$\delta'((\text{init},\text{back}), \X)
= ((\text{init},\text{copy rest}), \X, R).$$

\paragraph{Copying the rest of the input}
For all $\s \in\Sigma$,
let $$\delta'((\text{init},\text{copy rest}), \s)
= ((\text{init},\text{copy one}, \s), \X, R).$$

For all $\s \in\Sigma$,
for all $\gamma \in\Gamma'\setminus\{\,\0{}\,\}$,
let $$\delta'((\text{init},\text{copy one}, \s), \gamma)
= ((\text{init},\text{copy one}, \s), \gamma, R).$$

For all $\s \in\Sigma$,
let $$\delta'((\text{init},\text{copy one}, \s), \0{})
= ((\text{init},\text{back}), \A, L).$$

\paragraph{Start the simultation}
Let $$\delta'((\text{init},\text{copy rest}), \e)
= ((\text{simul},q_0), \e, R).$$
