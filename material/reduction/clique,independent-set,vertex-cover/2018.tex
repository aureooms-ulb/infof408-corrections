...
\begin{proof}
  ...
  case where $G$ contains no edge and $\bar{G}$ contains
  $\binom{\card{V}}{2}$ or $\frac{\card{V}(\card{V} - 1)}{2}$ which is the number of
  edges in a complete graph. By~\ref{cl:npc}, CLIQUE is in NP, that is, ``easy''. Since
  ...
  case where $G$ contains no edge and $\bar{G}$ contains
  $\binom{\card{V}}{2}$ or $\frac{\card{V}(\card{V} - 1)}{2}$ which is the number of
  edges in a complete graph. By~\ref{cl:npc}, CLIQUE is NP-hard, that is, ``hard''. Since
